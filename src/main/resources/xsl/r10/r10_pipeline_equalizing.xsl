...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <!-- Identity template, mode="s4_equalizing". -->
    <xsl:template match="node()" mode="s4_equalizing">
        <xsl:copy>
            <xsl:apply-templates select="node()" mode="s4_equalizing"/>
        </xsl:copy>
    </xsl:template>

    <!-- Root template, mode="s4_equalizing" -->
    <xsl:template match="/" mode="s4_equalizing">
        ...
        <xsl:variable name="first">
            <xsl:apply-templates mode="s4_equalizing">
                <xsl:with-param name="elements" tunnel="yes" select="$elements-localname-distinct"/>
            </xsl:apply-templates>
        </xsl:variable>
        
        <xsl:apply-templates mode="s4_equalizing_2" select="$first" use-when="true()"/>
    </xsl:template>

    <!-- Item template, mode="s4_equalizing" -->
    <xsl:template match="/*/*" mode="s4_equalizing">
    ...
    <!-- Identity template: mode="s4_equalizing_2" -->
    <xsl:template match="node()" mode="s4_equalizing_2">
        <xsl:copy>
            <xsl:apply-templates select="node()" mode="s4_equalizing_2"/>
        </xsl:copy>
    </xsl:template>

    <!-- Root template: mode="s4_equalizing_2" -->
    <xsl:template match="/" mode="s4_equalizing_2">
        <!-- this sub-step adds a children count attribute to each item element -->
        <xsl:variable name="children-count">
            <xsl:apply-templates mode="children-count"/>
        </xsl:variable>
        ...
        <xsl:apply-templates mode="s4_equalizing_2">
            <xsl:with-param name="elements" tunnel="yes" select="$elements-localname-distinct"/>
        </xsl:apply-templates>
    </xsl:template>

    <!-- Item template, mode="s4_equalizing_2" -->
    <xsl:template match="/*/*" mode="s4_equalizing_2">
        <xsl:param name="elements" tunnel="yes"/>
        <xsl:copy>
            <xsl:variable name="item" select="."/>
